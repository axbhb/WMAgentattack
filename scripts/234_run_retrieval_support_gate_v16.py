...
def _calibration(embedding,indices,task_names,k):
    values=[]
    for start in range(0,len(indices),512):
        batch=indices[start:start+512];similarity=embedding[batch]@embedding[indices].T
        for row,event_index in enumerate(batch):similarity[row,np.asarray([task_names[j]==task_names[event_index] for j in indices])]=-np.inf
...
def main():
    ...
    for fold in folds:
        events=v5._fold(source,fold);arrays=v5._arrays(events,source["candidate_catalog"],128);graphs=v12._graph_array(events,graph_dataset);embedding=_embedding(arrays,graphs,cfg);surfaces=v12._surfaces(events,arrays);task_names=[event["task_name"] for event in events]
        for horizon in horizons:
            surface=surfaces[horizon];train_mask=np.asarray([events[i]["split"]=="training" for i in surface["starts"]]);test_mask=~train_mask;train_starts=surface["starts"][train_mask];test_starts=surface["starts"][test_mask];train_targets=surface["targets"][train_mask];threshold=_calibration(embedding,train_starts,task_names,cfg["k"]);similarity,neighbor=_topk(embedding[test_starts],embedding[train_starts],cfg["k"]);neighbor_targets=train_targets[neighbor];legal=surface["legal"][test_mask,-1];targets=surface["targets"][test_mask];probabilities=np.zeros((len(test_starts),len(arrays["candidates"])),np.float64)
