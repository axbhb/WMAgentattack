...
ARCHIVE=/share/guozhix/wmagentattack/0724/observed_clean_state_pairing_repair/fixed_v1
...
{
  echo "execution_mode=remote_login_existing_clean_pairing_repair"
...
cp configs/0724_executed_call_pairing_repair_protocol.json \
...
sha256sum \
  configs/0724_executed_call_pairing_repair_protocol.json \
  src/wmagentattack/clean_state_instrumentation.py \
  src/wmagentattack/trace_execution_pairing.py \
  scripts/120_replay_observed_clean_state.py \
  tests/test_observed_clean_state_replay.py \
  tests/test_trace_execution_pairing.py \
  scripts/server/run_0724_observed_clean_pairing_repair.sh \
